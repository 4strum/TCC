...
$id = intval($_GET['id']);
$sql_code_emprestimo = "SELECT * FROM tb_emprestimo WHERE fk_id_livro = '$id'";
...
if (isset($_POST['Confirmar'])) {
  if ($emprestimo) {
    ?>
    <script>
      alert("Não é possível excluir o livro, pois está emprestado.");
      window.location = 'consemprestimo.php';
    </script>
    <?php
  } else {
    $sql_code = "DELETE FROM tb_livro WHERE id_livro= '$id'";
    $sql_query = $mysqli->query($sql_code) or die($mysqli->error);

    if ($sql_query) {
      ?>
      <script>
        alert("Livro foi deletado com sucesso!");
        window.location = 'conslivro.php';
      </script>
      <?php
    } else {
      ?>
  ...
  <div class="app-container">
    ...
    <div class="app-content">
      <div class="app-content-header">
        <h1 class="app-content-headerText">Exclusão de livro</h1>
      </div>
