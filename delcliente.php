  ...
  $id = intval($_GET['id']);
  $sql_code_emprestimo = "SELECT * FROM tb_emprestimo WHERE fk_id_cliente = '$id'";
  $sql_query_emprestimo = $mysqli->query($sql_code_emprestimo) or die($mysqli->error);
  $emprestimo = $sql_query_emprestimo->fetch_assoc();

  if (isset($_POST['Confirmar'])) {
    if ($emprestimo) {
      ?>
      <script>
        alert("Não é possível excluir o cliente, pois está com emprestimo pendente.");
        window.location = 'consemprestimo.php';
      </script>
      <?php
    } else {
      $sql_code = "DELETE FROM tb_cliente WHERE id_cliente= '$id'";
      ...
      if ($sql_query) {
        ...
        <script>
          ...
          window.location = 'conscliente.php';
        </script>
        <?php
      } else {
        ?>
...
<body>
  <div class="app-container">
    ...
    <div class="app-content">
      <div class="app-content-header">
        <h1 class="app-content-headerText">Exclusão de cliente</h1>
      </div>
  ...
  <script>
    document.getElementById('naoButton').addEventListener('click', function(event) {
      ...
      window.location = 'conscliente.php'; // Redireciona para onde desejar
    });
  </script>
    </div>
  </div>
